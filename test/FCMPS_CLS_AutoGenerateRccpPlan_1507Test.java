...
public class FCMPS_CLS_AutoGenerateRccpPlan_1507Test extends TestCase {
	String FOS_File="";
	String config_xml="";
	String FA_NO="FIC";
	int maxColorCount=4;  //型體每周的配色個數
	int FORE_PLAN_WEEKS=12; //可提前周數
	int SHOOT_MIN_PRODUCE_QTY=1000; //射出最小排產量
...
//    	FCMPS_CLS_AutoGenerateRccpPlanTest.doPublish("1421SC", "N", conn);
    	try {
        	...
        	FCMPS_CLS_AutoGenerateRccpPlan_1507 rccp1452=new FCMPS_CLS_AutoGenerateRccpPlan_1507();    	
        	rccp1452.do_Rccp(FA_NO, config_xml, FORE_PLAN_WEEKS, conn,SHOOT_MIN_PRODUCE_QTY);  
//    		double PLAN_QTY=FCMPS_PUBLIC.get_PROC_Planed_QTY(FA_NO, 1526, "200", 1526, conn);
//    		System.out.println(PLAN_QTY);
